...
import src.main.java.DataAbstractions.Item;
 ...
 * @author JONATHAN
 */
public class LeaveApplication extends Item {

    public LeaveApplication(List<String> Details, String Type, DataReader reader, DataWriter writer) {
    ...
    public List<Item> getDownwardsRelatedItems(String Type) {
        switch (Type) {
            case "PO", "PR", "Sales" -> {
                return this.getItemRelatedToLeaveApplication(Type);
            }
            default -> {
                return null;
            }
    ...
    private List<Item> getItemRelatedToLeaveApplication(String Type) {
...
//        Get All Product/PR/PO Rows Related To The PR/PO/Payment
        List<List<String>> ItemDetailList = this.reader.FitlerData("LeaveApplicationID", ID);
